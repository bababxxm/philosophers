...
/*   Updated: 2025/05/18 13:58:45 by sklaokli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	parse_args(t_table *t, int ac, char **av)
{
	...
	if (!parse_int(&t->time_to_sleep, av[3], MIN_MS, MAX_MS))
		return (broadcast(PERR_TME_SLP));
	if (ac != 5)
		t->meals_for_each = 1;
	else
	{
		if (!parse_int(&t->meals_for_each, av[4], MIN_MS, MAX_MS))
...
bool	broadcast(int perrno)
{
	...
	if (perrno == PERR_ARG_NON)
		...
		write(2, "invalid value for meals_count: expected 60-800 ms", 50);
