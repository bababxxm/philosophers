...
// ///////////////////////////////////////////////////////////////////
	...
	// printf("Philo no.%d go to Rong_A\n", p->order);
	if ( p->order % 2 == 0) // ku
	{
		printf("Philo no.%d 	go to pa_kapao\n", p->order);
		pthread_mutex_lock(&p->d->mutex_fork[p->fork_r]);
		printf("Philo no.%d 		taken fork right (fork_r=%d)\n", p->order, p->fork_r);

		pthread_mutex_lock(&p->d->mutex_fork[p->fork_l]);
		printf("Philo no.%d 		taken fork left	 (fork_l=%d)\n", p->order, p->fork_l);
		sleep(10);

		printf("Philo no.%d 			eat========\n", p->order);
